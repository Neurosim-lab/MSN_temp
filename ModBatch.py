...
def tupleToStr (obj):
    ...
    return obj
# -------------------------------------------------------------------------------
# function to convert ndarray to list (avoids erro when saving/loading)
# -------------------------------------------------------------------------------
def arrayToList(obj):
    if isinstance(obj, list):
        for i, item in enumerate(obj):
            if isinstance(item, (list, dict, specs.Dict, specs.ODict)):
                arrayToList(item)
            elif isinstance(item, np.ndarray):
                obj[i] = item.tolist()

    elif isinstance(obj, (dict, specs.Dict, specs.ODict)):
        for key, val in obj.iteritems():
            if isinstance(val, (list, dict, specs.Dict, specs.ODict)):
                arrayToList(val)
...
class Batch(object):
    ...
    def save(self, filename):
        ...
        dataSave = {'batch': arrayToList(tupleToStr(odict))} 
        if ext == 'json':
            import json
